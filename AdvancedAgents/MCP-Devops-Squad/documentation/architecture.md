...
## Overview
The **MCP-DevOps-Squad** is a multi-agent Site Reliability Engineering (SRE) system designed to automate monitoring, root-cause analysis, and incident remediation using Google Agent-to-Agent (A2A) patterns and the Model Context Protocol (MCP).
...
- **Function:** Analyzes metrics and logs passed from sub-agents to determine the best course of action. It delegates tasks to the Debugger or Janitor based on structured Pydantic `SREDecision` objects.
...
### 4. Janitor-Agent & Validator-Agent
...
2. **Lead-SRE** uses an LLM to analyze the data. If it requires more info, it delegates an `AgentTask` to **Debugger**.
3. **Debugger** returns its findings. **Lead-SRE** makes a final decision and delegates a fix task to **Janitor**.
